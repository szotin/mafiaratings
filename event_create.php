...
//	echo '<button class="icon" onclick="mr.createAddr(' . $club_id . ')" title="' . get_label('Create [0]', get_label('address')) . '"><img src="images/create.png" border="0"></button>';
	...
	$query = new DbQuery('SELECT id, name FROM systems WHERE club_id = ? ORDER BY name', $event->club_id);
	if ($row = $query->next())
	{
		echo '<tr><td>' . get_label('Rating system') . ':</td><td><select id="form-system"><option value="-1"';
