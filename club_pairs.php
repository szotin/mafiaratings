...
class Page extends ClubPageBase
{
	...
	{
		parent::prepare();
		check_permissions(PERMISSION_CLUB_MANAGER | PERMISSION_CLUB_REFEREE, $this->id);
	}
	...
	protected function show_body()
	{
		...
		$query = new DbQuery(
			'SELECT u1.id, nu1.name, u1.flags, cu1.flags,' .
			' u2.id, nu2.name, u2.flags, cu2.flags, p.policy' .
			' FROM pairs p' .
		...
		{
			list ($user1_id, $user1_name, $user1_flags, $user1_club_flags,
			      $user2_id, $user2_name, $user2_flags, $user2_club_flags, $policy) = $row;
			$pair = new stdClass();
			$pair->user1_id         = (int)$user1_id;
			$pair->user1_name       = $user1_name;
			$pair->user1_flags      = (int)$user1_flags;
			$pair->user1_club_flags = (int)$user1_club_flags;
			$pair->user2_id         = (int)$user2_id;
			$pair->user2_name       = $user2_name;
			$pair->user2_flags      = (int)$user2_flags;
			$pair->user2_club_flags = (int)$user2_club_flags;
			$pair->policy           = (int)$policy;
			$pair->source           = get_label('Global');
			$pairs_map[$user1_id . '_' . $user2_id] = $pair;
		}
		...
		while ($row = $query->next())
		{
			...
			{
				$pairs_map[$key]->policy = (int)$policy;
				$pairs_map[$key]->source = get_label('In this club');
			}
			else
			{
				$pair = new stdClass();
				...
				$pair->user2_club_flags = (int)$user2_club_flags;
				$pair->policy           = (int)$policy;
				$pair->source           = get_label('In this club');
				$pairs_map[$key]        = $pair;
			}
		}

		$pairs = array_values(array_filter($pairs_map, function($p) { return $p->policy != PAIR_POLICY_NOTHING; }));
		usort($pairs, function($a, $b) { return $a->user1_id - $b->user1_id; });

		$user_pic = new Picture(USER_CLUB_PICTURE, new Picture(USER_PICTURE));
		...
		foreach ($pairs as $pair)
		{
			...
			$user_pic->
				set($pair->user1_id, $pair->user1_name, $pair->user1_club_flags, 'c' . $this->id)->
			...
			echo '<td><table class="transp" width="100%"><tr><td width="52">';
			$user_pic->
				set($pair->user2_id, $pair->user2_name, $pair->user2_club_flags, 'c' . $this->id)->
