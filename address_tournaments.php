...
class Page extends AddressPageBase
{
	protected function show_body()
	{
		...
		$query = new DbQuery(
			'SELECT t.id, t.name, t.flags, t.start_time, ct.timezone, t.langs, a.id, a.address, a.flags,' .
			...
			$condition);
		$query->add($order_by . ' LIMIT ' . ($_page * PAGE_SIZE) . ',' . PAGE_SIZE);
		...
		while ($row = $query->next())
		{
			...
			list (
				$tournament->id, $tournament->name, $tournament->flags, $tournament->time, $tournament->timezone, $tournament->languages, 
		...
		foreach ($tournaments as $tournament)
		{
			...
			echo '<td><b><a href="tournament_standings.php?bck=1&id=' . $tournament->id . '">' . $tournament->name . '</b>';
