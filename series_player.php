...
class Page extends SeriesPageBase
{
	...
	protected function show_body()
	{
		...
		echo '<div class="tab">';
		echo '<button ' . ($this->view == VIEW_TOURNAMENTS ? 'class="active" ' : '') . 'onclick="goTo({view:' . VIEW_TOURNAMENTS . ',page:0})">' . get_label('Tournaments') . '</button>';
		echo '<button ' . ($this->view == VIEW_GAMES ? 'class="active" ' : '') . 'onclick="goTo({view:' . VIEW_GAMES . ',page:0})">' . get_label('Games') . '</button>';
		echo '<button ' . ($this->view == VIEW_STATS ? 'class="active" ' : '') . 'onclick="goTo({view:' . VIEW_STATS . ',page:0})">' . get_label('Stats') . '</button>';
		echo '</div>';
...
?>
		...
		{
			goTo({result: $('#result').val(), filter: checkboxFilterFlags(), page: 0});
		}
		
		function rolesChanged()
		{
			goTo({roles: $('#roles').val(), page: 0});
		}
<?php	
	}
}
