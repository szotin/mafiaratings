...
class Page extends AddressPageBase
{
	protected function show_body()
	{
		...
		$query = new DbQuery(
			'SELECT g.id, m.id, m.name, m.flags, g.start_time, g.end_time - g.start_time, g.result, g.video_id, g.canceled, e.id, e.name, e.flags, t.id, t.name, t.flags FROM games g' .
		...
		{
			list ($game_id, $moder_id, $moder_name, $moder_flags, $start, $duration, $game_result, $video_id, $is_canceled, $event_id, $event_name, $event_flags, $tournament_id, $tournament_name, $tournament_flags) = $row;
			...
			echo '<a href="view_game.php?address_id=' . $this->id . '&id=' . $game_id . '&bck=1">' . get_label('Game #[0]', $game_id) . '</a>';
			if ($is_canceled)
			{
