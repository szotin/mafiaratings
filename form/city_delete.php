...
try
{
	...
	list($name) = Db::record(get_label('city'), 'SELECT n.name FROM cities c JOIN names n ON n.id = c.name_id AND (n.langs & '.$_lang.') <> 0 WHERE c.id = ?', $id);

	echo '<table class="dialog_form" width="100%">';
	echo '<tr><td width="300">'.get_label('Replace [0] with: ', $name).'</td><td>';
	$query = new DbQuery('SELECT c.id, n.name FROM cities c JOIN names n ON n.id = c.name_id AND (n.langs & '.$_lang.') <> 0 WHERE c.id <> ? ORDER BY n.name', $id);
