...
try
{
	...
	if ($event_id > 0)
	{
		list($club_id, $tournament_id, $city_id, $area_id) = Db::record(get_label('event'), 
			'SELECT c.id, t.id, ct.id, ct.area_id'.
			' FROM events e'.
	...
	else 
	{
		...
		{
			$club_id = (int)$_profile->user_club_id;
		}
		else if (count($_profile->clubs) > 0)
		{
			reset($_profile->clubs);
			$club_id = current($_profile->clubs)->id;
		}
		else
		{
			list($club_id) = Db::record(get_label('club'), 'SELECT MIN(id) FROM clubs');
		}
		list($city_id, $area_id) = Db::record(get_label('user'), 
			'SELECT ct.id, ct.area_id'.
			' FROM users u'.
