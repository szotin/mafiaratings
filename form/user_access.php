...
try
{
	...
	else if (isset($_REQUEST['club_id']))
	{
		...
		list($name, $user_flags) = Db::record(get_label('club'), 'SELECT c.name, cu.flags FROM club_regs cu JOIN clubs c ON c.id = cu.club_id WHERE cu.club_id = ? AND cu.user_id = ?', $club_id, $user_id);
