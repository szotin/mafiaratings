...
class Page extends EventPageBase
{
	protected function show_body()
	{
		...
		echo '<tr class="th darker"><td width="58"><button class="icon" onclick="mr.eventCreateBroadcast(' . $this->event->id . ')" title="' . get_label('Create broadcast for [0]', $this->event->name) . '"><img src="images/create.png"></button></td>';
		...
		$query = new DbQuery('SELECT day_num, table_num, part_num, url FROM event_broadcasts WHERE event_id = ? ORDER BY day_num, table_num, part_num', $this->event->id);
		while ($row = $query->next())
		{
			list ($day, $table, $part, $url) = $row;
			echo '<tr><td><button class="icon" onclick="mr.eventDeleteBroadcast(' . $this->event->id . ',' . $day . ',' . $table . ',' . $part . ', \'' . get_label('Are you sure you want to remove the broadcast?') . '\')"><img src="images/delete.png"></button>';
