...
class Page extends SeriesPageBase
{
	...
	protected function show_body()
	{
		...
		$condition = new SQL(' AND g.result > 0 AND st.series_id = ?', $this->id);
		...
		if ($this->user_id > 0)
		{
			$pos_query = new DbQuery('SELECT u.id, count(g.id) FROM users u JOIN games g ON g.moderator_id = u.id JOIN series_tournaments st ON st.tournament_id = g.tournament_id WHERE u.id = ?', $this->user_id, $condition);
			...
			if ($row = $pos_query->next())
			{
				...
				if ($u_games > 0)
				{
					$pos_query = new DbQuery('SELECT count(*) FROM (SELECT u.id FROM users u JOIN games g ON g.moderator_id = u.id JOIN series_tournaments st ON st.tournament_id = g.tournament_id WHERE TRUE', $condition);
		...
		list ($count) = Db::record(get_label('user'), 'SELECT count(DISTINCT g.moderator_id) FROM games g JOIN users u ON u.id = g.moderator_id JOIN series_tournaments st ON st.tournament_id = g.tournament_id WHERE TRUE', $condition);
