...
class Page extends TournamentPageBase
{
	protected function show_body()
	{
		...
		list ($count) = Db::record(get_label('user'), 'SELECT count(DISTINCT moderator_id) FROM games WHERE club_id = ? AND canceled = FALSE AND result > 0', $this->id);
		...
		$query = new DbQuery(
			'SELECT u.id, u.name, u.flags, SUM(IF(g.result = 1, 1, 0)), SUM(IF(g.result = 2, 1, 0)), c.id, c.name, c.flags' . 
				...
				' WHERE g.tournament_id = ? AND canceled = FALSE AND result > 0' .
