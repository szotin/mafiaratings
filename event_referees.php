...
class Page extends EventPageBase
{
	protected function show_body()
	{
		...
		$query = new DbQuery(
			'SELECT u.id, nu.name, u.flags, SUM(IF(g.result = ' . GAME_RESULT_TOWN . ', 1, 0)) AS civ, SUM(IF(g.result = ' . GAME_RESULT_MAFIA . ', 1, 0)) AS maf, SUM(IF(g.result = ' . GAME_RESULT_TIE . ', 1, 0)) AS tie,' .
		...
		{
			++$number;
			list ($id, $name, $flags, $civil_wins, $mafia_wins, $ties, $club_id, $club_name, $club_flags, $user_nickname, $event_user_flags, $tournament_user_flags, $club_user_flags) = $row;
