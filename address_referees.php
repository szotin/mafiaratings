...
class Page extends AddressPageBase
{
	...
	protected function show_body()
	{
		...
		show_date_filter();
		echo '&emsp;&emsp;';
		show_checkbox_filter(array(get_label('tournament games'), get_label('rating games'), get_label('canceled games')), $this->filter, 'filterChanged');
		...
		if ($this->user_id > 0)
		{
			$pos_query = new DbQuery('SELECT count(g.id) FROM users u JOIN games g ON g.moderator_id = u.id JOIN events e ON e.id = g.event_id', $condition);
			...
			if ($row = $pos_query->next())
			{
				...
				if ($u_games > 0)
				{
					$pos_query = new DbQuery('SELECT count(*) FROM (SELECT u.id FROM users u JOIN games g ON g.moderator_id = u.id JOIN events e ON e.id = g.event_id', $condition);
		...
		list ($count) = Db::record(get_label('user'), 'SELECT count(DISTINCT g.moderator_id) FROM games g JOIN events e ON e.id = g.event_id', $condition);
