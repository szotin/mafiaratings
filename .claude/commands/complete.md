...
5. Deploy every staged non-SQL file via FTP to `public_html/`, preserving the file's relative path. Use the FTP host/user/password stored in the auto-memory (project_ftp_deploy). Do NOT deploy `CLAUDE.md` (see project_deployment memory) and do NOT try to deploy files under `db/`.
