...
class ApiPage extends GetApiPageBase
{
	...
	protected function get_help()
	{
		...
		$help->request_param('club', 'Club id. For example: <a href="scores.php?club=1"><?php echo PRODUCT_URL; ?>/api/get/scores.php?club=1</a> returns the scores in Vancouver Mafia Club. Club scoring system is used by default.', '-');
		$help->request_param('game', 'Game id. For example: <a href="scores.php?game=1299"><?php echo PRODUCT_URL; ?>/api/get/scores.php?game=1299</a> returns the scores of the game 1299, played in VaWaCa-2017 tournament. VaWaCa-2017 event scoring system is used by default because this game belongs to this event.', '-');
		$help->request_param('event', 'Event id. For example: <a href="scores.php?event=7927"><?php echo PRODUCT_URL; ?>/api/get/scores.php?event=7927</a> returns the scores of VaWaCa-2017 tournament. VaWaCa-2017 event scoring system is used by default.', '-');
		$help->request_param('address', 'Address id. For example: <a href="scores.php?address=10"><?php echo PRODUCT_URL; ?>/api/get/scores.php?address=10</a> returns the scores of the games played in Tafs Cafe in Vancouver Mafia Club. Vancouver Mafia Club scoring system is used by default.', '-');
		$help->request_param('city', 'City id. For example: <a href="scores.php?city=49"><?php echo PRODUCT_URL; ?>/api/get/scores.php?city=49</a> returns the scores of the games played in Seattle. Default scoring system is used by default (currently it is ФИИМ).', '-');
		$help->request_param('area', 'City id. The difference with city is that when area is set, the games from all nearby cities are also returned. For example: <a href="scores.php?area=1"><?php echo PRODUCT_URL; ?>/api/get/scores.php?area=1</a> returns the scores of the games played in Vancouver and nearby cities like Delta, Burnaby, Surrey, etc. Though <a href="scores.php?city=1"><?php echo PRODUCT_URL; ?>/api/get/scores.php?city=1</a> returns only the scores in Vancouver itself. Default scoring system is used by default (currently it is ФИИМ).', '-');
		$help->request_param('country', 'Country id. For example: <a href="scores.php?country=2"><?php echo PRODUCT_URL; ?>/api/get/scores.php?country=2</a> returns the sores of the games played in Russia. Default scoring system is used by default (currently it is ФИИМ).', '-');
		$help->request_param('with_user', 'User id. For example: <a href="scores.php?with_user=4"><?php echo PRODUCT_URL; ?>/api/get/scores.php?with_user=4</a> returns the scores for the games where lilya participated. Default scoring system is used by default (currently it is ФИИМ).', '-');
		$help->request_param('langs', 'Languages filter. 1 for English; 2 for Russian. Bit combination - 3 - means both (this is a default value). For example: <a href="scores.php?langs=1"><?php echo PRODUCT_URL; ?>/api/get/scores.php?langs=1</a> returns the scores for English games Default scoring system is used by default (currently it is ФИИМ).', '-');
		$help->request_param('role', 'The score for the specified role only. Possible values are:
				<ul>
					<li>a - all roles (default)</li>
					<li>r - red roles = civilian + sheriff: <a href="scores.php?role=r"><?php echo PRODUCT_URL; ?>/api/get/scores.php?role=r</a></li>
					<li>b - black roles = mafia + don: <a href="scores.php?role=b"><?php echo PRODUCT_URL; ?>/api/get/scores.php?role=b</a></li>
					<li>c - civilian but not the sheriff: <a href="scores.php?role=c"><?php echo PRODUCT_URL; ?>/api/get/scores.php?role=c</a></li>
					<li>s - sheriff: <a href="scores.php?role=s"><?php echo PRODUCT_URL; ?>/api/get/scores.php?role=s</a></li>
					<li>m - mafia but not the don: <a href="scores.php?role=m"><?php echo PRODUCT_URL; ?>/api/get/scores.php?role=m</a></li>
					<li>d - don: <a href="scores.php?role=d"><?php echo PRODUCT_URL; ?>/api/get/scores.php?role=d</a></li>
				</ul>', '-');
		$help->request_param('number', 'Number in the game (1-10). For example: <a href="scores.php?number=2&event=7927"><?php echo PRODUCT_URL; ?>/api/get/scores.php?number=2&event=7927</a> returns scores earned by players playing on number 2 in VaWaCa-2017 tournement. In the other words: who is the best number 2 in the tournament.', '-');
		$help->request_param('before', 'Unix timestamp. For example: <a href="scores.php?before=1483228800"><?php echo PRODUCT_URL; ?>/api/get/scores.php?before=1483228800</a> returns scores earned before 2017. In other words it returns scores as they were at the end of 2016.', '-');
		...
		$help->request_param('page_size', 'Page size. Default page_size is ' . API_DEFAULT_PAGE_SIZE . '. For example: <a href="scores.php?club=1&page_size=32"><?php echo PRODUCT_URL; ?>/api/get/scores.php?club=1&page_size=32</a> returns top 32 players for Vancouver Mafia Club; <a href="scores.php?club=6&page_size=0"><?php echo PRODUCT_URL; ?>/api/get/scores.php?club=6&page_size=0</a> returns all players for Empire of Mafia club in one page; <a href="scores.php?club=1"><?php echo PRODUCT_URL; ?>/api/get/scores.php?club=1</a> returns top ' . API_DEFAULT_PAGE_SIZE . ' players for Vancouver Mafia Club;', '-');
