...
class ApiPage extends GetApiPageBase
{
	protected function prepare_response()
	{
		...
		{
			$condition->add(' AND t.start_time < ?', get_datetime($started_before, $_profile->timezone)->getTimestamp());
		}

		if (!empty($ended_before))
		{
			$condition->add(' AND t.start_time + t.duration < ?', get_datetime($ended_before, $_profile->timezone)->getTimestamp());
		}

		if (!empty($started_after))
		{
			$condition->add(' AND t.start_time >= ?', get_datetime($started_after, $_profile->timezone)->getTimestamp());
		}

		if (!empty($ended_after))
		{
			$condition->add(' AND t.start_time + t.duration >= ?', get_datetime($ended_after, $_profile->timezone)->getTimestamp());
		}
		...
		if ($user_id > 0)
		{
			$condition->add(' AND t.id IN (SELECT e.tournament_id FROM players p JOIN games g ON g.id = p.game_id JOIN events e ON e.id = g.event_id WHERE p.user_id = ? AND g.canceled = FALSE AND g.result > 0)', $user_id);
		}
		...
		{
			$query = new DbQuery(
				'SELECT t.id, t.name, t.flags, t.langs, a.id, a.name, a.flags, c.id, c.name, c.flags, t.start_time, t.duration, t.notes, t.price, t.scoring_id, t.rules, l.id, l.name, l.flags, ct.timezone FROM tournaments t' . 
		...
		{
			$query = new DbQuery(
				'SELECT t.id, t.name, t.flags, t.langs, t.address_id, t.club_id, t.start_time, t.duration, t.notes, t.price, t.scoring_id, t.rules, t.league_id FROM tournaments t' . 
			...
			while ($row = $query->next())
			{
				...
				$tournament->timestamp = (int)$tournament->timestamp;
				$tournament->duration = (int)$tournament->duration;
				$tournament->scoring_id = (int)$tournament->scoring_id;
