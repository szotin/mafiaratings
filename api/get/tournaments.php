...
class ApiPage extends GetApiPageBase
{
	protected function prepare_response()
	{
		...
		if ($user_id > 0)
		{
			$condition->add(' AND t.id IN (SELECT e.tournament_id FROM players p JOIN games g ON g.id = p.game_id JOIN events e ON e.id = g.event_id WHERE p.user_id = ?)', $user_id);
		}
