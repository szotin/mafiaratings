...
class ApiPage extends GetApiPageBase
{
	protected function prepare_response()
	{
		...
		if ($user_id > 0)
		{
			$condition->add(' AND t.id IN (SELECT e.tournament_id FROM players p JOIN games g ON g.id = p.game_id JOIN events e ON e.id = g.event_id WHERE p.user_id = ? AND (g.flags & '.GAME_FLAG_CANCELED.') = 0)', $user_id);
		}
	...
	protected function get_help()
	{
		...
		$help->request_param('rules_code', 'Rules code. For example: <a href="tournaments.php?rules_code=' . DEFAULT_RULES . '">' . PRODUCT_URL . '/api/get/tournaments.php?rules_code=' . DEFAULT_RULES . '</a> returns all tournaments where the rules with the code ' . DEFAULT_RULES . ' was used. Please check <a href="rules.php?help">' . PRODUCT_URL . '/api/get/rules.php?help</a> for the meaning of rules codes and getting rules list.', '-');
