...
class ApiPage extends OpsApiPageBase
{
	...
	function delete_op()
	{
		...
		Db::exec(get_label('game'), 'UPDATE rebuild_ratings SET game_id = ? WHERE game_id IN (SELECT id FROM games WHERE event_id = ?)', $prev_game_id, $event_id); // it also triggers rebuilding mr_bonus_stats
	...
	// import_dimtom
	//-------------------------------------------------------------------------------------------------------
	private function fill_seating_mapping($misc, $event_id, $tournament_id)
	{
		...
		if (count($available_spots) < count($regs))
		{
			if (is_null($tournament_id))
			{
				...
				$scoring->id = (int)$scoring_id;
				$scoring->version = (int)$scoring_version;
			...
			else
			{
				...
				if (!is_null($normalizer))
				{
					...
					$normalizer->id = (int)$normalizer_id;
					$normalizer->version = (int)$normalizer_version;
				}
	...
	function import_dimtom_op()
	{
		...
		foreach ($dimtom->rounds as $r)
		{
			...
			{
				$round[] = array_values((array)$dimtom->games[$r->gameIds[$i]]->players);
			}
			$rounds[] = $round;
		}
		...
		if ($dimtomDef->players != $event_players || $dimtomDef->tables != $event_tables || $dimtomDef->games != $event_games)
		{
			...
			Db::exec(get_label('event'), 'UPDATE events SET players = ?, tables = ?, games = ? WHERE id = ?',
				$dimtomDef->players, $dimtomDef->tables, $dimtomDef->games, $event_id);
			$event_players = $dimtomDef->players;
		...
		$hash = $dimtomDef->hash;
		$rounds = SeatingDef::applyMapping($rounds, $restrict_mapping);
		$rounds = $dimtomDef->renumberByDistribution($rounds);

		// --- 7. Store seating in seatings table if not present ---
		Db::begin();
		if (!(new DbQuery('SELECT 1 FROM seatings WHERE hash = ?', $hash))->next())
		{
			$ps = $dimtomDef->calculatePlayersScore($rounds);
			$ns = $dimtomDef->calculateNumbersScore($rounds);
			$ts = $dimtomDef->calculateTablesScore($rounds);
			Db::exec(get_label('seating'),
				...
				$hash, json_encode($rounds), $ps, $ns, $ts);
		}
		Db::commit();
		...
		$this->assign_seating_to_event($misc, $hash, $rounds, $final_mapping);
	...
	function import_dimtom_op_help()
	{
		...
		$help->request_param('update_event', 'Optional. If 1, update the event\'s players/tables/games to match the seating file instead of returning a confirmation request.');
		return $help;
	}
	...
	//-------------------------------------------------------------------------------------------------------
	// assign_seating_to_event — shared helper used by set_seating_op and import_dimtom_op
	//-------------------------------------------------------------------------------------------------------
	private function assign_seating_to_event($misc, $hash, $seating, $mapping)
	{
	...
	//-------------------------------------------------------------------------------------------------------
	// load_reg_users_and_pairs — loads registered players with distances and classifies tournament pairs.
	// Returns [$reg_users, $restrictions, $welcome_pairs, $avoid_pairs].
	//-------------------------------------------------------------------------------------------------------
	private function load_reg_users_and_pairs($event_id, $tournament_id, $club_id, $event_players, $event_round)
	{
		global $_lang;
	...
	//-------------------------------------------------------------------------------------------------------
	// build_slot_mapping — assigns registered users to seating slots.
	// Same algorithm as set_seating_op: restricted pairs first, then welcome/avoid, then by distance+rating.
	// Returns $final_mapping [seating_slot => user_id].
	//-------------------------------------------------------------------------------------------------------
	private function build_slot_mapping($seating, $event_players, $restrict_mapping, $reg_users, $welcome_pairs, $avoid_pairs)
	{
