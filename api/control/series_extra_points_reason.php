...
class ApiPage extends ControlApiPageBase
{
	protected function prepare_response()
	{
		...
		if (empty($term))
		{
			$query = new DbQuery('SELECT reason, count(*) c FROM series_extra_points GROUP BY reason ORDER BY c DESC, reason');
		}
		else
		{
			$query = new DbQuery('SELECT reason, count(*) c FROM series_extra_points WHERE reason LIKE ? GROUP BY reason ORDER BY c DESC, reason', '%' . $term . '%');
		}
		if ($num > 0)
		{
			$query->add(' LIMIT ' . $num);
...
?>
		...
		<dt>series</dt>
			<dd>Series id. Only the users who were participating in this series are returned. For example <a href="user.php?series=5">/api/control/user.php?series=5</a> users who played on VaWaCa-2017 tournament.</dd>
		<dt>club</dt>
