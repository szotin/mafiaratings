...
//		$this->event->init($this->event->club_id, -1);
		...
		if (isset($_POST['update']))
		{
			...
			$this->event->update();
			throw new RedirectExc('create_event_mailing.php?msg=1&events=' . $this->event->id);
		}
	}
	...
	protected function show_body()
	{
		...
		$query = new DbQuery('SELECT id, name FROM systems WHERE club_id = ? ORDER BY name', $this->event->club_id);
		if ($row = $query->next())
		{
			echo '<tr><td class="dark">' . get_label('Rating system') . ':</td><td><select name="system"><option value="-1"';
