...
class Page extends TournamentPageBase
{
	...
	protected function show_body()
	{
		...
		$video_count = 0;
		$column_count = 0;
		$can_add = is_permitted(PERMISSION_CLUB_MEMBER | PERMISSION_TOURNAMENT_MANAGER | PERMISSION_TOURNAMENT_REFEREE, $this->club_id, $this->id);
		...
		list ($count) = Db::record(get_label('video'), 'SELECT count(*) FROM videos v WHERE v.tournament_id = ?', $this->id, $condition);
		...
		if ($can_add)
		{
			...
			echo '<tr><td align="center" width="' . COLUMN_WIDTH . '%"><a href="#" onclick="mr.createVideo(' . $this->video_type . ', null, null, ' . $this->id , ')">';
