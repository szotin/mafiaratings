...
class Page extends GeneralPageBase
{
	...
	protected function show_body()
	{
		...
		case CCCF_CLUB:
			...
			{
				$condition->add(' AND g.club_id IN (SELECT club_id FROM user_clubs WHERE (flags & ' . UC_FLAG_BANNED . ') = 0 AND user_id = ?)', $_profile->user_id);
			}
			break;
		case CCCF_CITY:
		...
		$query = new DbQuery(
			...
			' FROM players p JOIN games g ON p.game_id = g.id', $condition);
		...
		$ref = 'by_numbers.php?ccc=' . $this->ccc_filter->get_code();
		...
		echo '<td width="100" align="center">' . sorting_link($ref, SORT_TYPE_BY_WIN, get_label('Games won (%)')) . '</td>';
