...
class Page extends AddressPageBase
{
	...
	protected function show_body()
	{
		...
		$query = new DbQuery(
			...
			' WHERE e.address_id = ? AND v.type = ? AND (v.lang & ?) <> 0 ORDER BY g.start_time DESC, v.post_time DESC, v.id DESC LIMIT ' . ($_page * $page_size) . ',' . $page_size, $this->id, $this->video_type, $langs);
