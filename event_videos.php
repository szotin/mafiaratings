...
class Page extends EventPageBase
{
	protected function show_body()
	{
		...
		list ($count) = Db::record(get_label('video'), 'SELECT count(*) FROM videos WHERE event_id = ? AND type = ? AND (lang & ?) <> 0', $this->event->id, $video_type, $langs);
		...
		if ($can_add)
		{
			...
			echo '<tr><td align="center" width="' . COLUMN_WIDTH . '%"><a href="#" onclick="mr.createVideo(' . $video_type . ', ' . $this->event->club_id . ', ' . $this->event->id , ')">';
		...
		$query = new DbQuery(
			...
			' WHERE v.event_id = ? AND v.type = ? AND (v.lang & ?) <> 0 ORDER BY v.post_time DESC, v.id DESC LIMIT ' . ($_page * $page_size) . ',' . $page_size, $this->event->id, $video_type, $langs);
		while ($row = $query->next())
		{
			...
			echo '<a href="video.php?bck=1&id=' . $video_id . '&event_id=' . $this->event->id . '&vtype=' . $video_type . '&langs=' . $langs . '"><img src="https://img.youtube.com/vi/' . $video . '/0.jpg" width="200" title="' . $title . '">';
			if (!is_valid_lang($this->event->langs))
			{
