...
class Page extends EventPageBase
{
	protected function show_body()
	{
		...
		echo '>&nbsp;</td><td width="48">'.get_label('Tournament').'</td><td width="48">'.get_label('Moderator').'</td><td width="48">'.get_label('Result').'</td><td width="48">'.get_label('Video').'</td></tr>';
		$query = new DbQuery(
			'SELECT g.id, g.flags, ct.timezone, m.id, m.name, m.flags, g.start_time, g.end_time - g.start_time, g.result, g.video_id, g.canceled, t.id, t.name, t.flags, l.id, l.name, l.flags FROM games g' .
		...
		{
			list ($game_id, $game_flags, $timezone, $moder_id, $moder_name, $moder_flags, $start, $duration, $game_result, $video_id, $is_canceled, $tournament_id, $tournament_name, $tournament_flags, $league_id, $league_name, $league_flags) = $row;
			...
			echo '<a href="view_game.php?event_id=' . $this->event->id . '&id=' . $game_id . '&bck=1">' . get_label('Game #[0]', $game_id) . '</a>';
			if ($is_canceled)
			{
