	...
	protected function show_body()
	{
		...
		switch ($this->view)
		{
			case VIEW_SCORINGS:
				...
				echo '<tr class="darker"><td width="48"><a href="#" onclick="mr.createScoringSystem(-1)" title="'.get_label('New scoring system').'">';
				echo '<img src="images/create.png" border="0"></a></td>';
				
				echo '<td>' . get_label('Scoring system name') . '</td></tr>';
				
				$query = new DbQuery('SELECT id, name, version FROM scorings WHERE club_id IS NULL AND league_id IS NULL ORDER BY name');
			...
			case VIEW_NORMALIZERS:
				echo '<div class="tabcontent">';
				echo '<table class="bordered light" width="100%">';
				echo '<tr class="darker"><td width="48"><a href="#" onclick="mr.createNormalizer(-1)" title="'.get_label('New scoring normalizer').'">';
				echo '<img src="images/create.png" border="0"></a></td>';
				
				echo '<td>' . get_label('Scoring normalizer name') . '</td></tr>';
				
				$query = new DbQuery('SELECT id, name, version FROM normalizers WHERE club_id IS NULL AND league_id IS NULL ORDER BY name');
				while ($row = $query->next())
				{
					list ($id, $name, $version) = $row;
					echo '<tr><td class="dark" align="center">';
			...
			case VIEW_GAININGS:
				echo '<div class="tabcontent">';
				echo '<table class="bordered light" width="100%">';
				echo '<tr class="darker"><td width="48"><a href="#" onclick="mr.createGainingSystem(-1)" title="'.get_label('New gaining system').'">';
				echo '<img src="images/create.png" border="0"></a></td>';
