...
class Page extends TournamentPageBase
{
	...
	protected function show_body()
	{
		...
		list($this->games_count) = Db::record(get_label('game'), 'SELECT count(*) FROM games g WHERE g.tournament_id = ? AND (g.flags & '.GAME_FLAG_CANCELED.') = 0', $this->id, $condition);
		
		$civils_win_count = 0;
		$mafia_win_count = 0;
		$tie_count = 0;
		$query = new DbQuery('SELECT g.result, count(*) FROM games g WHERE g.tournament_id = ? AND (g.flags & '.GAME_FLAG_CANCELED.') = 0', $this->id, $condition);
		...
		if ($games_count > 0)
		{
			...
			list ($counter) = Db::record(get_label('game'), 'SELECT COUNT(DISTINCT p.user_id) FROM players p JOIN games g ON g.id = p.game_id WHERE g.tournament_id = ? AND (g.flags & '.GAME_FLAG_CANCELED.') = 0', $this->id);
			echo '<tr><td>'.get_label('People played').':</td><td>' . $counter . '</td></tr>';
			
			list ($counter) = Db::record(get_label('game'), 'SELECT COUNT(DISTINCT g.moderator_id) FROM games g WHERE g.tournament_id = ? AND (g.flags & '.GAME_FLAG_CANCELED.') = 0', $this->id);
			...
			list ($a_game, $s_game, $l_game) = Db::record(
				...
				'SELECT AVG(g.end_time - g.start_time), MIN(g.end_time - g.start_time), MAX(g.end_time - g.start_time) ' .
					'FROM games g WHERE (g.flags & '.GAME_FLAG_CANCELED.') = 0 AND g.tournament_id = ? AND g.end_time > g.start_time + 900 AND g.end_time < g.start_time + 20000', 
				$this->id);
		...
		if ($games_count > 0)
		{
			$query = new DbQuery('SELECT p.kill_type, p.role, count(*) FROM players p JOIN games g ON p.game_id = g.id WHERE g.tournament_id = ? AND (g.flags & '.GAME_FLAG_CANCELED.') = 0', $this->id);
