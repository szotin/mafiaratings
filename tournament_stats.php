...
class Page extends TournamentPageBase
{
	...
	protected function show_body()
	{
		...
		list($this->games_count) = Db::record(get_label('game'), 'SELECT count(*) FROM games g WHERE g.tournament_id = ? AND g.canceled = FALSE AND g.result > 0', $this->id);
		
		$playing_count = 0;
		$civils_win_count = 0;
		$mafia_win_count = 0;
		$query = new DbQuery('SELECT g.result, count(*) FROM games g WHERE g.tournament_id = ? AND g.canceled = FALSE AND g.result > 0', $this->id);
