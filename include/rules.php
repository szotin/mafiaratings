...
define('RULES_NIGHT_POSE_FIIM', 0); //	4.5.1. Игроки сидят скрестив руки на груди, наклонив голову под углом примерно 45 градусов.
...
define('RULES_NIGHT_SEQ_FIIM', 0); // 5.4.1. Знакомство с Доном (5.2), затем Договоренка мафии (5.1), затем Знакомство с Шерифом (5.3).
...
define('RULES_VOTING_FIIM', 0); // 7.10.1. Перед голосованием игроки должны убрать руки с игрового стола. Для того, чтобы проголосовать против того или иного игрока, игрок обязан после озвучивания кандидатуры незамедлительно поставить кулак с выставленным вверх большим пальцем на игровой стол. 
...
define('RULES_LEGACY_ANY', 2); // 8.17.3. Может назвать тройку черных, однако она никак не будет зафиксирована Судьей и не повлияет на результаты турнира..
...
define('RULES_EXTRA_POINTS_FIIM', 0); // 8.20.1. Судья и его помощники назначают дополнительные баллы игрокам повлиявшим на исход игры. Баллы могут быть как положительными, так и отрицательными. Они назначаются в соответствии с регламентом турнира.
...
// firstKillGesturing
define('RULES_FIRST_KILL_GESTURING', 22);
define('RULES_FIRST_KILL_GESTURING_OK', 0); // 10.11.2. Не наказывается.
define('RULES_FIRST_KILL_GESTURING_WARNING', 1); // 10.11.1. Фол.

// 10.21. Просьба назвать или не называть кого-либо в "завещание", выраженная вслух или с помощью жестикуляции.
// legacyAdvice
define('RULES_LEGACY_ADVICE', 23);
define('RULES_LEGACY_ADVICE_KILL', 0); // 10.21.1. Удаление.
define('RULES_LEGACY_ADVICE_WARN', 1); // 10.21.2. Фол.
define('RULES_LEGACY_ADVICE_OK', 2); // 10.21.3. Не наказывается.

// 10.22. Апелляция к влиянию на "завещание" с целью доказательства игровой роли или объяснения игровых действий.
// legacyArgument
define('RULES_LEGACY_ARGUMENT', 24);
define('RULES_LEGACY_ARGUMENT_KILL', 0); // 10.22.1. Удаление.
define('RULES_LEGACY_ARGUMENT_WARN', 1); // 10.22.2. Фол.
define('RULES_LEGACY_ARGUMENT_OK', 2); // 10.22.3. Не наказывается.
...
$_rules_options = array(
	array('noKillOnFour', array('red', 'draw'), 0, 5, false),
	array('nightPose', array('fiim', 'maf-club', 'free'), 3, 4, false),
	array('nightSeq', array('fiim', 'maf-club', 'quick'), 4, 3, false),
	array('rotation', array('first', 'last'), 5, 2, false),
	array('nominationThumb', array(false, true), 6, 2, false),
	array('withdraw', array('allowed', 'auto', 'no'), 6, 4, false),
	array('nominationQuestion', array(true, false), 6, 6, false),
	array('votingPose', array('fiim', 'maf-club'), 6, 9, false),
	array('firstVoting', array('standard', 'to-talk'), 6, 15, false),
	array('altSplit', array('continue', 'revote'), 6, 18, false),
	array('killAll', array('no', 'draw'), 6, 19, false),
	array('splitOnFour', array(true, false), 6, 20, true),
	array('shooting', array('all', 'alive'), 7, 2, false),
	array('killedNom', array(false, true), 7, 5, false),
	array('sheriffResponse', array('nod', 'thumb', 'both', 'either'), 7, 12, false),
	array('legacy', array(true, false), 7, 16, false),
	array('gameEndSpeech', array(true, false), 7, 18, false),
	array('extraPoints', array('fiim', 'maf-club', 'best-player', 'best-player-move'), 7, 19, false),
	array('threeWarningsNomination', array(true, false), 8, 5, false),
	array('antimonster', array('no-voting', 'no', 'nominated', 'participation'), 8, 9, false),
	array('earlyVote', array('warn', 'ok'), 9, 8, false),
	array('undertableSign', array('warn', 'ok'), 9, 9, false),
	array('firstKillGesturing', array('ok', 'warn'), 9, 10, false),
	array('legacyAdvice', array('ok', 'warn', 'kill'), 9, 20, false),
	array('legacyArgument', array('ok', 'warn', 'kill'), 9, 21, false),
	array('pry', array('loss', 'kill'), 9, 23, false),
	array('audienceTip', array('loss', 'kill'), 9, 24, false),
