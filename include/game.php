...
class Game
{
	...
	// Dummy player is not voting in mafiaratings javascript client.
	// In this format all players must vote if they are alive. This function fixes it.
	// Todo: make js client to fill all votes and get rid of this function.
	// function fix_dummy_player_votes()
	// {
		// for ($i = 0; $i < 10; ++$i)
		// {
			// $player = $this->data->players[$i];
			...
			// {
				...
				// {
					...
					// for ($voting_time->round = 0; $this->compare_gametimes($voting_time, $death_time) < 0; ++$voting_time->round)
					// {
						// if (!isset($player->voting))
						// {
							// $player->voting = array();
						// }
						// while ($voting_time->round >= count($player->voting))
						// {
							// $player->voting[] = NULL;
						// }
						// if ($player->voting[$voting_time->round] == NULL)
						// {
							// $votes = array(array());
							// for ($j = 0; $j < 10; ++$j)
							// {
								// $p = $this->data->players[$j];
								// if (isset($p->voting) && $voting_time->round < count($p->voting))
								// {
									// $v = $p->voting[$voting_time->round];
									// if ($v != NULL)
									// {
										// if (is_array($v))
										// {
											// for ($vr = 0; $vr < count($v); ++$vr)
											// {
												// if (!isset($votes[$vr]))
												// {
													// $votes[$vr] = array();
												// }
												// if (!isset($votes[$vr][$v[$vr]]))
												// {
													// $votes[$vr][$v[$vr]] = 0;
												// }
												// ++$votes[$vr][$v[$vr]];
							...
							// if (!empty($votes[0]))
							// {
								// for ($voting_time->votingRound = 0; $voting_time->votingRound < count($votes) - 1 && $this->compare_gametimes($voting_time, $death_time) >= 0; ++$voting_time->votingRound)
								// {
									// $count = 0;
									// foreach ($votes[$voting_time->votingRound + 1] as $n => $c)
									// {
										// $count = max($votes[$voting_time->votingRound][$n], $count);
									// }
									
									// $candidate = 0;
									// foreach ($votes[$voting_time->votingRound + 1] as $n => $c)
									// {
										// if ($votes[$voting_time->votingRound][$n] < $count)
										// {
											// $candidate = $n;
											// break;
										// }
										// else if ($votes[$voting_time->votingRound][$n] == $count)
										// {
											// $candidate = $n;
										// }
									// }
									...
									// {
										// if ($player->voting[$voting_time->round] == NULL)
										// {
											// $player->voting[$voting_time->round] = array();
										// }
										// $player->voting[$voting_time->round][] = $candidate;
									// }
								// }
								
								// if ($this->compare_gametimes($voting_time, $death_time) < 0)
								// {
									// $winners = array();
									// for ($j = 0; $j < 10; ++$j)
									// {
										// $p = $this->data->players[$j];
										// if (isset($p->death) && $p->death->round == $voting_time->round && $p->death->type == DEATH_TYPE_DAY)
										// {
											// $winners[] = $j + 1;
										// }
									// }
									
									// switch (count($winners))
									// {
										// case 0:
											// $max_count = 0;
											// foreach ($votes[$voting_time->votingRound] as $n => $c)
											// {
										...
										// case 1:
											// if ($player->voting[$voting_time->round] == NULL)
											// {
												// $player->voting[$voting_time->round] = $winners[0];
											// }
											// else
											// {
												// $player->voting[$voting_time->round][] = $winners[0];
										...
										// default:
											...
											// {
												// if (isset($votes[$voting_time->votingRound][$w]))
												// {
													// $count = max($votes[$voting_time->votingRound][$w], $count);
												// }
											// }
											// $candidate = 0;
											// foreach ($winners as $w)
											// {
												// if (isset($votes[$voting_time->votingRound][$w]))
												// {
													// if ($votes[$voting_time->votingRound][$w] < $count)
													// {
														// $candidate = $w;
														// break;
													// }
													// else if ($votes[$voting_time->votingRound][$w] == $count)
													// {
														// $candidate = $w;
													// }
												// }
