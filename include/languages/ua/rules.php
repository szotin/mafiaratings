...
(
		...
		(
			...
			(
				...
				(
					...
					'Если в первый игровой «день» выставлена только одна кандидатура, то голосование не проводится. Если в голосовании победил только один игрок, то он умирает только в том случае, если проголосовал за себя сам. В остальных случаях он просто получает дополнительные 30 секунд для речи.',
				),
				array
				(
					...
					'Если выставлен только один, то голосования нет. Попил можно сломать только в себя.',
				),
			),
			...
			(
				...
				(
					...
					'Если в первый день (не путать с нулевым) за столом девять игроков и трое из них дважды набирают одинаковое число голосов, дальнейшее голосование не проводится. Судья объявляет «Наступает ночь». Никто не покидает игровой стол. В любой другой день поднимать троих можно.',
				),
				array
				(
					'Можно.',
	...
	(
		...
		(
			...
			'Существуют следующие наказания. В порядке увеличения строгости.<ul><li>Устное предупреждение.</li><li>Фол (также известный, как замечание, или предупреждение).</li><li>Технический фол.</li><li>Дисквалификация с игры. (Удаление).</li><li>Поражение команды.</li></ul>', // 9.3.
			...
			(
				RULES_3_WARNINGS,
				'Наказание за три замечания',
				array
				(
					'При получении трёх фолов игрок лишается права слова в свою ближайшую минуту. За исключением случаев, когда это его последнее слово или оправдательное слово при переголосовании. Игрок лишается слова один раз. В последующие дни игрок получит слово. У игрока получившего три фола остается право выставить кандидатуру на голосование. Когда за столом остается три или четыре игрока, игроку получившему три фола предоставляется «укороченная» речь 30 секунд. Если игрок с тремя фолами попал в ситуацию переголосования, то он имеет право слова. Он пропустит свое слово в последующие дни.', // 9.5.1.
					'При получении трёх фолов речь игрока в ближайшую минуту сокращается до 30 секунд. Речь сокращается один раз. В последующие дни игрок получит полные 30 секунд.', // 9.5.2.
					'При получении трёх фолов игрока не получает никакого наказания.', // 9.5.3.
				),
				array
				(
					'Потеря слова на один круг.',
					'Речь сокращается до 30 секунд на один круг.',
					'Нет наказания.',
				),
			),
			array // 9.6.
			(
				...
				array
				(
					'В этой версии правил технических фолов нет.', // 9.6.1.
					'За повышенные эмоции, споры с судьей, стуки по столу отвлекающие от игры, а также грубые высказывания в адрес других игроков, судья может дать технический фол. Технические фолы считаются отдельно от основных. За два технических фола игрок получает удаление.', // 9.6.2.
				),
				array
				(
					'Нет.',
