...
class Page extends SeriesPageBase
{
	...
	protected function show_body()
	{
		...
		$create_link = NULL;
		$condition = new SQL('a.tournament_id IN (SELECT tournament_id FROM series_tournaments WHERE series_id = ?)', $this->id);
		...
		case CCCF_CLUB:
			if ($ccc_id > 0)
			{
				$condition->add(' AND a.club_id = ?', $ccc_id);
			...
			{
				$condition->add(' AND a.club_id IN (' . $_profile->get_comma_sep_clubs() . ')');
			}
			break;
		case CCCF_CITY:
			$condition->add(' AND a.club_id IN (SELECT id FROM clubs WHERE city_id IN (SELECT id FROM cities WHERE id = ? OR area_id = ?))', $ccc_id, $ccc_id);
			break;
		case CCCF_COUNTRY:
			$condition->add(' AND a.club_id IN (SELECT c.id FROM clubs c JOIN cities ct ON ct.id = c.city_id WHERE ct.country_id = ?)', $ccc_id);
			break;
		}
